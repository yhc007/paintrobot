...
엣지는 폴링할 때마다 현재 PLC 모델을 보낸다. 서버는 이를 두 곳에 나눠 담는다:

| 대상 | 내용 | 빈도 |
|---|---|---|
| `paintrobot.plc_state` | 엣지당 **한 행**, 계속 덮어씀 (살아있음 + 현재 모델) | 폴링마다 |
| `paintrobot.jobs` | `plc_only` 행 = **실제 모델 전환** | 전환 때만 |

모델이 직전과 같으면 `jobs`에 쓰지 않고 `{"unchanged":1}`로 응답한다. 예전에는
폴링마다 행이 쌓여 하루 7,758건이었는데 실제 전환은 7회였다 (1,108:1).

`GET /api/v1/plc/current`는 `plc_state`를 읽는다 — 엣지당 한 행이라 상수 시간.
예전에는 오늘치 `jobs`를 전부 훑었다. 마이그레이션 006 이전 빌드와 섞여도
죽지 않도록, 테이블이 없거나 비면 예전 스캔으로 되돌아간다.

> `reconcile`이 쓰는 PLC 타임라인은 영향받지 않는다. 원래도 `plc_timeline()`이
> 연속 중복을 압축했으므로, 전환만 저장하면 압축이 불필요해질 뿐 결과는 같다.
